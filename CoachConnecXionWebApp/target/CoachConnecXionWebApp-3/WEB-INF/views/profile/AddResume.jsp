...
<head>
	<title>Add Coach/Trainer Resume</title>
	<meta http-equiv="Content-Type" content="text/html;charset=utf-8" />
	<meta charset="utf-8" />
	<meta name="description" content="Allows coaches to add resumes to provide insight of their background" />
...
function validation(){
	...
	var i=0;
	if(value1==""){
		document.getElementById('errorMessage1').innerHTML="File Name Should not empty";
		i=1;
	}else{
...
<%-- END BODY No Side test --%>
	...
	<c:url var="resumeUrl" value="/addResume" />
	<form:form method="POST" action="${resumeUrl}"
		modelAttribute="resumeAttribute" enctype="multipart/form-data">
		<input type="hidden" name="profileId" id="profileId" value="${profileId}"></input>
		
		<table style="width: 100%;">
			<tr>
				<td style="text-align: left; width: 200px;"><label
					for="fileName">Name of the Document:</label></td>
	...
	<table style="margin-top: -31px; margin-left: 0px;">
		<tr>
			<td>
				<c:url var="profileUrl" value="/public/profile" /> 
				<form:form action="${profileUrl}" method="GET">
					<input type="hidden" name="cuin" value="<%= SecurityUtils.getCurrentUser() == null?0:SecurityUtils.getCurrentUser().getId()%>" >
					<input type="submit"
