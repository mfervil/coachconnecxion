										...
										<c:forEach items="${cities}" varStatus="status" var="cities">
											...
											<c:choose>
	    										...
	    										<c:when test="${count == 2 && itemsperrow > 1}"> <%-- This is to handle All Coaches scenario --%>
														<a href="${pageContext.request.contextPath}/tutor/public/find-a-coach-by-city-state/coachtypeid/${coachtypeid}/coachtype/${coachtype}/statecode/${statecode}/state/${state}/cityname/<%=Constants.DEFAULT_URL_STRING %>/pagenumber/1"> ${cities.city}</a>
														<br>
	    										</c:when>
											<c:when test="${count <= itemsperrow}">
													<a href="${pageContext.request.contextPath}/tutor/public/find-a-coach-by-city-state/coachtypeid/${coachtypeid}/coachtype/${coachtype}/statecode/${statecode}/state/${state}/cityname/${cities.city}/pagenumber/1"> ${cities.city} coaches</a>
													<br>
											</c:when>	
											</c:choose>	
										</c:forEach>
